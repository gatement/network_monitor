...
{
	$("#fromDateTimeField").val(nm.utils.getTimeStr(nm.utils.getTimeByHour(-1))); // 1 hour ago
	$("#toDateTimeField").val(nm.utils.getTimeStr()); // now
...
function refreshImages(showSpiner)
{
	...
	var fromDateTime = nm.utils.getTimeStr(nm.utils.getTimeByMinute(-minutes));
	var toDateTime = nm.utils.getTimeStr(); // now
	...
	window.ajaxContainer["time"] = $.ajax({
		...
		success: function(data, textStatus, jqXHR)
		{
			if(data.success)
			{
				...
				$("#lastUpdated").text("Last updated: " + nm.utils.getTimeStr());
			}
			else
			{
