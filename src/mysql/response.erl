...
-import(conn).
-import(my_io).
-import(packet_parser, [parse_server_response_packet/4]).
...
%% Result = list(#ok_packet{} | #ok_stmt_packet{} | #error_packet{} | #rs_header{} | #field_metadata{} | #rs_row_data{} | #eof_packet{})
%% 
...
handle_response(Connection, Initial_metadata, Stmt) -> 
  Is_compress = Connection#connection.client_info#client_options.compress =:= 1,
  case Packet_list = my_io:receive_data(Connection#connection.socket, Is_compress) of
...
%      handle_response(State, Packet_list, New_metadata, Result, Stmt);
    ...
    #field_metadata{} ->
      New_metadata = Metadata#metadata{field_metadata = [R|Metadata#metadata.field_metadata]},
      handle_response(State, Packet_list, New_metadata, Result, Stmt);
    #rs_row_data{} -> handle_response(State, Packet_list, Metadata, [R|Result], Stmt);
