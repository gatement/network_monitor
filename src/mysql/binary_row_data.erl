...
%% API Functions
%%
-spec parse_binary_row(Binary::binary(), Metadata::#metadata{}) -> #rs_row_data{} | #mysql_error{}.

%% @spec parse_binary_row(Binary::binary(), Metadata::#metadata{}) -> #rs_row_data{} | #mysql_error{}
%% @doc function converts binary representation of a packet to rs_row_data record. 
...
parse_binary_row(B, #metadata{field_count = N} = Metadata) -> 
  ...
  <<NULL_bit_map:Bit_size/bits, _:2>> = util:reverse_binary(Null_bitmap),
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_VARCHAR}, _Flags) ->
  util:extract_length_coded_string(B);
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_NEWDECIMAL}, _Flags) ->
  {String, B1} = util:extract_length_coded_string(B),
...
draw_value(B, #field_metadata{type = ?MYSQL_TYPE_BLOB}, {Is_binary, _, _, _}) ->
  if
    Is_binary -> util:extract_length_coded_binary(B);
    true      -> util:extract_length_coded_string(B)
  end;
