...
%% mysql_type() = integer()
%% 
%% @doc The function converts list of constants represented of MySQL types of prepared statement parameters to binary(). 
...
param_to_binary(?MYSQL_TYPE_INT24, P) -> util:string_to_length_coded_binary(erlang:integer_to_list(P, 10));
param_to_binary(?MYSQL_TYPE_INT24 + ?UNSIGNED, P) -> util:string_to_length_coded_binary(erlang:integer_to_list(P, 10));
...
param_to_binary(?MYSQL_TYPE_TIME, P) -> util:time_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_TIMESTAMP, P) -> util:datetime_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_DATETIME, P) -> util:datetime_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_DATE, P) -> util:date_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_YEAR, P) -> util:string_to_length_coded_binary(erlang:integer_to_list(P, 10));
param_to_binary(?MYSQL_TYPE_VARCHAR, P) -> util:string_to_length_coded_binary(P);
param_to_binary(T, P) when T =:= ?MYSQL_TYPE_VAR_STRING; T =:= ?MYSQL_TYPE_STRING ->
  if
    is_binary(P) ->  util:binary_to_length_coded_binary(P);
    true ->   util:string_to_length_coded_binary(P)
  end;
param_to_binary(?MYSQL_TYPE_ENUM, P) -> util:string_to_length_coded_binary(atom_to_list(P));
param_to_binary(?MYSQL_TYPE_SET, P) -> util:string_to_length_coded_binary(util:atom_list_to_string(P));
...
param_to_binary(?MYSQL_TYPE_DECIMAL, P) -> util:decimal_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_NEWDECIMAL, P) -> util:decimal_to_lc_binary(P);
param_to_binary(?MYSQL_TYPE_BIT, P) -> 
  BitSize = bit_size(P),
  Size = ((BitSize + 7) div 8) * 8 - BitSize,
  util:binary_to_length_coded_binary(<<0:Size, P:BitSize/bitstring>>);
param_to_binary(_, _) -> <<>>
.
...
check_error(I, Message) ->
  #mysql_error{type = statement, message = "Parameter #" ++ integer_to_list(I) ++ Message, 
                source = "statement:check_param/3"}
.
